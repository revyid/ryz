  ...
  return (
    <div className="relative w-full font-sans">
        ...
        <nav className="mx-auto max-w-7xl relative">
          ...
          <div className="bg-gray-900/80 rounded-xl shadow-lg border border-gray-700 text-white flex items-center justify-between h-16 px-4 md:px-6 relative overflow-hidden navbar-glow">
            ...
            <div className="hidden md:flex items-center space-x-8 absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2">
              {navItems.map((item) => (
                <Link 
                  key={item.label} 
                  href={item.href} 
                  className="text-slate-200 hover:text-white text-sm font-medium relative group py-1.5"
                >
            ...
            <div className="flex items-center gap-3">
                  ...
                  {isSignedIn ? (
                    ...
                    <SignInButton mode="modal">
                      <motion.button 
                        ...
                        whileTap={{ scale: 0.95 }} 
                        className="bg-white/5 text-sm font-medium px-3 py-2 rounded-lg transition-all duration-300 hover:bg-white/10 border border-white/10 flex items-center gap-1.5"
                      >
                        <span>Sign in</span>
                      </motion.button>
                    </SignInButton>
              ...
              <motion.button 
                className="md:hidden bg-white/5 p-2 rounded-lg text-slate-200 hover:text-white transition-all hover:bg-white/10 border border-white/10" 
                  ...
                  <div className="flex flex-col p-2 space-y-1">
                    {navItems.map((item, index) => (
                      ...
                      >
                        <Link 
                          href={item.href} 
                          className="text-slate-200 hover:text-white text-sm font-medium py-2.5 px-3 rounded-lg hover:bg-white/5 transition-all flex items-center justify-between" 
      ...
      <AnimatePresence>
        {isChatOpen && (
          <motion.div 
            className="fixed top-24 md:top-20 inset-x-0 z-40 px-4 md:px-0 md:right-6 md:left-auto md:w-[450px]" 
            initial={{ opacity: 0, y: 20, scale: 0.95 }} 
            animate={{ opacity: 1, y: 0, scale: 1 }} 
            exit={{ opacity: 0, y: 20, scale: 0.95 }} 
                ...
                <div className="flex items-center justify-between p-4 border-b border-gray-700">
                  ...
                  <div className="flex items-center gap-1">
                    <motion.button 
                      ...
                      whileTap={{ scale: 0.9 }} 
                      className="text-slate-400 hover:text-white p-1.5 rounded-full hover:bg-white/5 transition-all" 
                      title="Clear chat"
                    >
                      <Eraser size={14}/>
                    </motion.button>
                ...
                <div className="h-[350px] md:h-[500px] overflow-y-auto px-4 py-5 space-y-4">
                  <AnimatePresence initial={false}>
                    {chatMessages.map((msg) => (
                      <motion.div 
                        key={msg.id} 
                        className={`flex items-start gap-2.5 group ${msg.type === 'bot' ? 'max-w-[85%]' : 'ml-auto max-w-[85%] flex-row-reverse'}`} 
                        initial={{ opacity: 0, y: 10 }} 
                        animate={{ opacity: 1, y: 0 }} 
                        transition={{ duration: 0.3 }}
                      >
                        <div className={`flex-shrink-0 w-7 h-7 rounded-full flex items-center justify-center ${
                          msg.type === 'bot' ? 'bg-indigo-500/30' : 'bg-blue-500/30'
                        }`}>
                          {msg.type === 'bot' ? (
                        ...
                        <div className="relative">
                          <div className={`py-2.5 px-3.5 rounded-2xl text-sm ${
                            msg.type === 'bot' 
                  ...
                  <form ref={formRef} onSubmit={handleSubmit}>
                    <div className="relative">
                      <textarea 
                        ...
                        className="w-full bg-gray-800/50 text-white placeholder-slate-400 rounded-lg py-2.5 px-3.5 pr-12 focus:outline-none focus:ring-1 focus:ring-indigo-500/50 border border-gray-600 resize-none text-sm transition-all duration-200 max-h-[200px]"
                        style={{ minHeight: '44px' }}
                      />
                      <motion.button 
                        type="submit"
                        disabled={!chatMessage.trim() || isLoading} 
                        whileHover={chatMessage.trim() && !isLoading ? { scale: 1.05 } : {}} 
                        whileTap={chatMessage.trim() && !isLoading ? { scale: 0.95 } : {}} 
