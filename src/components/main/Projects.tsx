...
export default function Projects() {
    ...
    return (
        <section ref={sectionRef} id="projects" className="py-32 px-4 sm:px-6 lg:px-8 text-slate-100 relative overflow-hidden">
            ...
            <div className="max-w-7xl mx-auto relative">
                    ...
                    <h2 className="relative text-5xl md:text-6xl font-bold inline-flex overflow-hidden">
                        <div className="flex items-center justify-center">
                            {titleLetters.map((letter, index) => (
                                <span 
                                    key={index} 
                                    className={`inline-block bg-clip-text text-transparent bg-gradient-to-r from-cyan-400 to-purple-400 transition-all duration-700 ${
                                        isInView
                        ...
                        <div 
                            className={`absolute -bottom-3 left-0 h-1 bg-gradient-to-r from-cyan-500 via-purple-500 to-transparent rounded-full transition-all duration-1000 delay-300 ease-out ${
                                isInView ? "w-full" : "w-0"
                            }`}
                        />
                    </h2>
                    <p 
                        className={`mt-6 text-lg text-slate-400 max-w-xl mx-auto transition-all duration-1000 delay-500 ease-out ${
                ...
                {featuredProject && (
                    <div className={`mb-20 transition-all duration-1000 ${isInView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-20"}`}>
                                ...
                                <div className="relative lg:w-3/5 overflow-hidden">
                                    ...
                                    <div className="absolute bottom-4 right-4 flex flex-wrap gap-2 justify-end">
                                        {featuredProject.tags.map((tag, idx) => (
                                            <span 
                                                key={idx} 
                                                className={`px-3 py-1 rounded-full text-xs font-medium bg-slate-800/90 backdrop-blur-sm border border-slate-700/70 text-${tag.color} transition-all duration-300 hover:scale-105 hover:shadow-glow-${tag.color}`}
                                            >
                                                {tag.name}
                                            </span>
                                        ))}
                                ...
                                <div className="p-8 lg:w-2/5 flex flex-col justify-center">
                                    ...
                                    <div className="mt-auto">
                                        <a 
                                            ...
                                            className="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-cyan-500 to-purple-500 hover:from-cyan-600 hover:to-purple-600 text-white font-medium rounded-xl transition-all duration-300 group-hover:shadow-glow"
                                        >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
                    {projects.map((project) => (
                        <div 
                            key={project.id} 
                            className={`group relative overflow-hidden rounded-xl backdrop-blur-sm bg-slate-900/40 border border-slate-800 hover:border-cyan-500/50 transition-all duration-500 transform ${
                                animatedProject >= project.id ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"
                            } hover:shadow-project`} 
                        ...
                        >
                            <div className="relative aspect-[16/10] overflow-hidden">
                                ...
                                <div className={`absolute inset-0 flex items-center justify-center bg-slate-900/70 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-opacity duration-300`}>
                                    <a 
                                        ...
                                        target="_blank" 
                                        rel="noopener noreferrer" 
                                        className="px-5 py-3 rounded-lg bg-cyan-500 hover:bg-cyan-600 text-white font-medium transition-all duration-300 flex items-center gap-2 transform translate-y-4 group-hover:translate-y-0"
                                    >
                            ...
                            <div className="p-6">
                                ...
                                <div className="flex flex-wrap gap-2 mt-auto">
                                    {project.tags.map((tag, idx) => (
                                        <span 
                                            key={idx} 
                                            className={`px-2 py-1 rounded-full text-xs font-medium border border-slate-700 text-${tag.color} bg-slate-800/50`}
                                        >
                                            {tag.name}
                                        </span>
                                    ))}
                            ...
                            <div className="absolute top-0 right-0 w-20 h-20 overflow-hidden pointer-events-none">
                                <div 
                                    className={`absolute top-0 right-0 w-10 h-10 translate-x-1/2 -translate-y-1/2 rotate-45 bg-gradient-to-br ${
                                        activeProject === project.id
                                            ? "from-cyan-500/30 via-cyan-500/20 to-transparent"
                                            : "from-slate-700/30 via-slate-700/20 to-transparent"
