...
export default function FloatingNavbar({ navItems }: FloatingNavbarProps) {
    ...
    const [chatMessages, setChatMessages] = useState<ChatMessage[]>([
        { type: 'bot', text: 'Hello! I\'m your AI assistant powered by OpenRouter. How can I help you today?', id: 'initial-message' },
    ]);
    ...
    const sendToOpenRouter = async (messages: OpenRouterMessage[]): Promise<string> => {
        if (!OPENROUTER_API_KEY) {
            return "I cannot connect to the AI service at the moment. Please check your OpenRouter API key configuration.";
        }
        try {
    ...
    return (<div className="relative w-full font-sans">
      ...
      <header className="fixed top-0 left-0 right-0 z-50 transition-all duration-700 px-4 py-3 md:px-6" style={{
        ...
        <nav ref={navbarRef} className="mx-auto max-w-7xl relative">
          ...
          <div className="backdrop-blur-xs bg-black/30 rounded-xl shadow-[0_8px_30px_rgb(0,0,0,0.12)] border border-white/10 text-white flex items-center justify-between h-16 px-4 md:px-6 relative overflow-hidden">
          ...
          <AnimatePresence>
            {isMobileMenuOpen && (<motion.div className="md:hidden mt-2" initial={{ opacity: 0, y: -20, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -20, scale: 0.95 }} transition={{ duration: 0.2, ease: [0.23, 1, 0.32, 1] }}>
                <div className="backdrop-blur-xs bg-black/30 rounded-xl shadow-[0_8px_30px_rgb(0,0,0,0.12)] border border-white/10 overflow-hidden">
                  <div className="flex flex-col p-2 space-y-1">
                    ...
                    {isLoaded && isSignedIn && (<motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: navItems.length * 0.05, duration: 0.2 }}>
                        <div className="text-slate-300 hover:text-white text-sm font-medium py-2.5 px-3 rounded-lg hover:bg-white/5 transition-all flex items-center justify-between border-t border-white/5 mt-2 pt-2 cursor-pointer" onClick={() => {
                    setIsMobileMenuOpen(false);
                }}>
                          <span>Sign out</span>
      ...
      <AnimatePresence>
        {isChatOpen && (<motion.div className="fixed top-24 md:top-20 inset-x-0 z-40 px-4 md:px-0 md:right-6 md:left-auto md:w-[450px]" initial={{ opacity: 0, y: 20, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 20, scale: 0.95 }} transition={{
            ...
            <div className="mx-auto max-w-md md:max-w-none relative">
              ...
              <div className="backdrop-blur-xs bg-black/50 rounded-xl shadow-[0_8px_30px_rgb(0,0,0,0.12)] border border-white/10 overflow-hidden relative">
                ...
                <div className="flex items-center justify-between p-4 border-b border-white/5">
                  <h3 className="font-medium text-white flex items-center gap-2.5">
                    ...
                    <div>
                      <div className="text-sm font-semibold text-white">OpenRouter AI</div>
