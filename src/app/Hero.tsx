...
const Hero = () => {
    ...
    return (<section id="home" ref={heroRef} className={`min-h-screen flex items-center justify-center pt-24 pb-12 px-4 sm:px-6 lg:px-8 text-slate-100 overflow-hidden relative ${isLoaded ? "opacity-100" : "opacity-0"} transition-all duration-1000`}>
      ...
      <div className="absolute inset-0 -z-10 overflow-hidden">
        ...
        <div className="particles absolute inset-0">
          {[...Array(30)].map((_, i) => (<div key={i} className={`absolute rounded-full animate-twinkle ${i % 3 === 0 ? "w-1.5 h-1.5 bg-indigo-300/30" :
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-16">
            ...
            <div className={`overflow-hidden ${isLoaded ? 'animate-reveal-delay-1' : ''}`}>
              <h2 className="text-2xl md:text-3xl font-medium text-slate-300 mb-6 flex items-center transform transition-all duration-700 delay-300">
                ...
                <div className="relative ml-4">
                  <span className="absolute inset-0 animate-ping-slow rounded-full bg-green-400 opacity-25"></span>
                  <span className="relative inline-block w-3 h-3 bg-green-400 rounded-full"></span>
                </div>
              </h2>
            </div>
          ...
          <div className="md:w-1/2 flex justify-center relative">
            <div className={`relative ${isLoaded ? 'animate-float-up' : 'translate-y-10 opacity-0'} transition-all duration-1000 delay-500`}>
                    ...
                    <div className="absolute bottom-6 left-6 text-white text-sm font-medium">
                      <div className="flex items-center">
                        <div className="relative">
                          <span className="absolute inset-0 animate-ping-slow rounded-full bg-green-400 opacity-25"></span>
                          <span className="relative inline-block w-2 h-2 bg-green-400 rounded-full"></span>
                        </div>
              ...
              <div className="absolute -bottom-6 left-1/2 -translate-x-1/2 bg-slate-800/90 backdrop-blur-sm px-6 py-3 rounded-xl shadow-lg border border-slate-700/50 flex items-center gap-3 transform hover:scale-105 transition-all duration-300">
                <div className="relative">
              ...
              <div className="absolute -right-8 top-1/4 w-16 h-16 bg-indigo-500/10 backdrop-blur-md rounded-xl border border-indigo-500/20 shadow-lg animate-float-slow animate-rotate-slow transform translate-z-30" style={getParallaxStyle(15)}></div>
      ...
      <div className={`absolute bottom-8 left-1/2 -translate-x-1/2 ${isLoaded ? 'animate-fade-in-up3' : 'opacity-0'} transition-all duration-700 delay-1200`}>
        ...
        <p className="text-white/50 text-xs font-medium mt-2 text-center group-hover:text-white/70 transition-all duration-300">Scroll Down</p>
      </div>
    </section>);
};
export default Hero;
